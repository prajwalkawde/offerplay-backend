...
exports.getAdjoeStats = exports.handleAdjoePostback = void 0;
const database_1 = require("../config/database");
const questController_1 = require("./questController");
const logger_1 = require("../utils/logger");
const client_1 = require("@prisma/client");
// GET /api/adjoe/postback  — called by adjoe servers (no auth)
const handleAdjoePostback = async (req, res) => {
    try {
        const { user_id, session_id, minutes, game_id, game_name, } = req.query;
        if (!user_id || !session_id) {
            return res.status(400).send('Bad Request');
        }
        // Idempotency check
        const existingSession = await database_1.prisma.adjoeSession.findUnique({
            where: { sessionId: String(session_id) },
        });
        if (existingSession?.status === 'credited') {
            logger_1.logger.warn('adjoe duplicate postback:', session_id);
            return res.send('OK');
        }
        ...
        if (ticketsEarned > 0 || coinsBonus > 0) {
            await database_1.prisma.$transaction(async (tx) => {
                ...
                if (coinsBonus > 0) {
                    ...
                    await tx.transaction.create({
                        data: {
                            userId,
                            type: client_1.TransactionType.ADJOE_BONUS,
                            amount: coinsBonus,
            ...
            await (0, questController_1.updateQuestProgress)(userId, 'PLAY_MINUTES', minutesPlayed);
            logger_1.logger.info(`adjoe credited: ${userId} - ${minutesPlayed}min → ${ticketsEarned} tickets + ${coinsBonus} coins`);
        }
        return res.send('OK');
    }
    catch (err) {
        logger_1.logger.error('adjoe postback error:', err);
        return res.status(500).send('ERROR');
    }
};
exports.handleAdjoePostback = handleAdjoePostback;
// GET /api/adjoe/stats  — authenticated user
const getAdjoeStats = async (req, res) => {
    try {
        const userId = req.user.id;
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const [todaySessions, allSessions] = await Promise.all([
            database_1.prisma.adjoeSession.findMany({ where: { userId, startedAt: { gte: today } } }),
            database_1.prisma.adjoeSession.aggregate({
                where: { userId },
...
exports.getAdjoeStats = getAdjoeStats;
