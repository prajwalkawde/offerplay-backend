...
export async function createIplQuestion(req: Request, res: Response): Promise<void> {
  const { matchId, question, options, points, correctAnswer, difficulty, category, language } = req.body as {
    matchId: string; question: string; options: string[]; points?: number;
    correctAnswer?: string; difficulty?: string; category?: string; language?: string;
  };

  if (!matchId || !question || !options || options.length < 2) {
    error(res, 'matchId, question, and at least 2 options are required', 400);
    return;
  }
