...
export async function generateIPLQuestions(req: Request, res: Response): Promise<void> {
  ...
  const {
    ...
    generateQuestionsFromScorecard,
    translateQuestions,
  } = await import('../services/claudeAiService');
  const { fetchMatchScorecard } = await import('../services/cricketService');
  ...
  (async () => {
    try {
      ...
      if (scorecard) {
        // ── NEW FLOW: real scorecard → Haiku English → parallel translate ──
        const matchDateStr = match.matchDate.toDateString();
        const venueName = match.venue ?? 'TBD';

        let englishQuestions: any[] = [];
        try {
        ...
        if (englishQuestions.length > 0) {
          ...
          const langQuestionsMap: Record<string, any[]> = { en: englishQuestions };

          if (isAllLangs) {
            const otherLangs = ALL_LANGUAGES.filter(l => l !== 'en');
            const translations = await Promise.allSettled(
              otherLangs.map(lang => translateQuestions(englishQuestions, lang))
            );
            translations.forEach((result, idx) => {
              const lang = otherLangs[idx];
              langQuestionsMap[lang] = result.status === 'fulfilled' ? result.value : englishQuestions;
              if (result.status === 'rejected') logger.error(`[QuestionGen] Translation to ${lang} failed:`, result.reason);
            });
          } else if (targetLanguages[0] !== 'en') {
            // Single non-English: translate once, drop English
            const lang = targetLanguages[0];
            try {
              langQuestionsMap[lang] = await translateQuestions(englishQuestions, lang);
            } catch (err) {
              logger.error(`[QuestionGen] Translation to ${lang} failed:`, err);
              langQuestionsMap[lang] = englishQuestions;
            }
            delete langQuestionsMap['en'];
          }

          // Save questions for each target language
          for (const lang of targetLanguages) {
            const questions: any[] = langQuestionsMap[lang] ?? langQuestionsMap['en'] ?? [];
            ...
            try {
                ...
                questions.map((q: any, i: number) =>
                  prisma.iplQuestion.create({
                    data: {
                      ...
                      category: q.category ?? 'trivia', difficulty: q.difficulty ?? 'medium',
                      status: 'active', isAutoGenerated: true, generatedBy: 'claude-haiku', approved: false,
                      questionNumber: i + 1,
              ...
              logger.info(`[QuestionGen] Saved ${questions.length} scorecard questions for lang: ${lang}`);
            } catch (saveErr) {
              logger.error(`[QuestionGen] Save failed for lang ${lang}:`, saveErr);
            }
          }
        } else {
          // Scorecard parse succeeded but generation failed — fall through to pre-match flow
          scorecard = null;
        }
      }

      if (!scorecard) {
        // ── FALLBACK: pre-match prediction questions (old flow) ──────────────
        for (const lang of targetLanguages) {
          try {
