...
async function testPubScale() {
  ...
  try {
    const response = await axios.post(
      ...
      { page: 1, size: 100, filt: [{ dim: 'platform', match: { type: 'any', value: ['android'] } }] },
      {
    ...
    const passBoth = offers.filter((o) =>
      isAvailableInIndia(o) && parseFloat(o.pyt?.amt || '0') >= MIN_PAYOUT_USD
    );
    ...
    passBoth.forEach((o: any, i: number) => {
    ...
    try {
      const earnRes = await axios.get('http://localhost:3000/api/earn/offers', {
